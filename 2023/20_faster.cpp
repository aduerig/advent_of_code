...
int main() {
    ...
    // while (pressed < 1001) {
        int low_rx_pulses = 0;
        // deque<bfs_node> iter_queue = my_queue_init;
        // deque<tuple<int, int, int>> iter_queue = my_queue_init;
        // veque::veque<tuple<int, int, int>> iter_queue = my_queue_init;

        // veque::veque<tuple<int, int, int>> iter_queue;
        // iter_queue.reserve(64);
        // for (auto& the_tuple: my_queue) {
        //     iter_queue.push_back(make_tuple(string_to_int[get<0>(the_tuple)], string_to_int[get<1>(the_tuple)], get<2>(the_tuple)));
        // }
        ...
        while (!iter_queue.empty()) {
            ...
            // bfs_node node = iter_queue.front();
            // int from_name = node.from_name;
            // int name = node.name;
            // int signal = node.signal;
            // print_bfs_node(int_to_string, node);
            ...
            if (is_flip_flop[name]) {
                ...
                for (auto sub_name: all_output_ints[name]) {
                    ...
                    // iter_queue.push_back({
                    //     .from_name = name,
                    //     .name = sub_name,
                    //     .signal = memory_flips[name]
                    // });
                }
            }
            else {
    ...
    cout << "low: " << lows << ", high: " << highs << ", low * high: " << lows * highs << endl;
    return 0;
}


// cout << "Starting queue mine: " << endl;
// for (auto& node: my_queue_init) {
//     print_bfs_node(int_to_string, node);
// }
// cout << endl << endl;
